...
// A category is only ever used for grouping in the UI and for the mode gate.
// Behaviour lives in the flags below it, never in the category name.
const STRUCTURE_CATEGORIES = ['structure', 'defense'];
  ...
  for (let i = 0; i < roster.length; i++) {
    ...
    for (let p = 0; p < STRUCTURE_IDS.length; p++) {
      ...
      const held = tank && tank.inventory
        ...
        : 0;
      const copies = Number.isInteger(held)
        ? Math.min(held, MAX_PURCHASED_COPIES)
        : 0;
...
function wardMultiplier(structures, target) {
  ...
  for (let i = 0; i < structures.length; i++) {
    ...
    if (dist <= spec.wardRadius) mult *= spec.wardFactor;
  }
  return mult;
}
