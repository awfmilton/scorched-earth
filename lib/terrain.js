...
function generateTerrain(seed, width, height, config = {}) {
  ...
  let biome = biomes[rngInstance.int(0, biomes.length)];
  
  if (config.terrainStyle && config.terrainStyle !== 'random' && biomes.includes(config.terrainStyle)) {
    biome = config.terrainStyle;
  }
  ...
  if (config.hillCount === 'high') roughness *= 1.5;
  if (config.hillCount === 'low') roughness *= 0.6;

  if (config.flatness === 'high') {
    smoothingPasses = Math.floor(smoothingPasses * 2);
    persistence *= 1.2;
  }
  if (config.flatness === 'low') {
    smoothingPasses = Math.max(1, Math.floor(smoothingPasses / 2));
    persistence *= 0.8;
  }
