...
class RoomManager {
  ...
  rejoin(connectionId, { code, playerToken }) {
    ...
    if (room.phase === 'playing' && otherConnected) {
      ...
      // path nothing has advanced activeSlot off them. Left alone, the
      // room waits on a seat that may not act until the sweep's 3-minute
      // turn timeout fires. Advance the cursor now, exactly as
      // disconnect() does when the active player drops.
    ...
    if (room.phase === 'paused') {
      ...
      if (!otherConnected) {
        // Find the correct next active player (which could be the reconnecting player, if alive, or someone else)
        // We temporarily set activeSlot to a dummy value so nextTurn searches from the top,
        // OR we just leave it and let nextTurn advance from the previous cursor.
        nextTurnResult = this.nextTurn(room);
        cursorReassigned = true;
      } else {
        /*
