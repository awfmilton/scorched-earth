...
class RoomManager {
  ...
  // Real reclaim, not just the lazy read-time backstop: the verify fleet
  // demonstrated that an index pruned only on reads of DEAD ids never
  // shrinks in production (dead ids are never read again) — 200k rejoin
  // cycles leaked 128MB. Every path that retires a connectionId or a room
  // must unbind explicitly.
  unbindConnection(connectionId, room) {
    const set = this.roomsByConnection.get(connectionId);
    if (!set) return;
    if (room === undefined) {
      this.roomsByConnection.delete(connectionId);
      return;
    }
    set.delete(room);
  ...
  start(connectionId, config) {
    ...
    // True until the first accepted gameplay input. A virgin round is fully
    // described by its seed, so a rejoiner can rebuild it exactly — no
    // spectating, no re-seed needed.
    room.roundVirgin = true;
  ...
  disconnect(connectionId) {
    ...
    const rooms = this.getRoomsByConnection(connectionId);
    if (rooms.length === 0) {
      this.unbindConnection(connectionId);
    ...
    // A closed socket never sends again: rejoin re-finds the room by CODE
    // and playerToken, never through this id, so the whole key retires.
    this.unbindConnection(connectionId);
    return { replies, broadcasts };
  }
  ...
  nextTurn(room) {
    ...
    if (alivePlayers.length <= 1) {
      ...
      if (matchOver) {
        ...
        room.phase = 'shopping';
        room.readyForNextRound = new Set();
        room.shoppingStartedAt = Date.now();
      }
  ...
  rejoin(connectionId, { code, playerToken }) {
    ...
    const supersededId = player.connectionId;
    player.connectionId = connectionId;
    player.connected = true;
    this.bindConnection(connectionId, room);
    if (supersededId && supersededId !== connectionId) {
      this.unbindConnection(supersededId, room);
    }
    ...
    if (room.phase === 'playing' && otherConnected && !room.roundVirgin) {
      // A VIRGIN round (no shot, drive or teleport accepted since it was
      // seeded) is fully described by its seed: the rejoiner rebuilds the
      // identical world and takes a full seat — sidelining them would be
      // pure loss, and after a re-seed it wedged the room when the first
      // returner dropped again.
      //
      // Past that point, spectating only makes sense while someone else can
      // still act. If every OTHER seat is dead, dropped or already
      // spectating, marking this one too would leave nextTurn() nothing to
      // advance to — the room parked itself with players connected,
      // unreachable by both the sweep's un-wedge (playing-only) and its
      // reaper (connected players block it). The rejoiner instead becomes
      // the reference client, the same exception the parked-room path below
      // has always made.
    ...
    if (room.phase === 'paused') {
      if (!otherConnected) {
         ...
         * be out of step with. Two cases:
         *
         * VIRGIN round, or a quick blip (parked under RESEED_MIN_PARK_MS):
         * plain resume. A virgin round's seed world IS the real world, and
         * a quick returner usually still holds their real board — resuming
         * is exact in the first case and right in almost every second one.
         *
         * DIRTY round parked long enough that everyone truly walked away:
         * re-seed the CURRENT round. The old resume-as-reference-client
         * rebuilt a FICTION — pristine terrain and full-hp hulls standing
         * in for a half-fought round — and every later rejoiner spectated
         * against it. A fresh seed gives everyone who returns the same
         * REAL world. Kits, bankrolls, rounds-won and the round counter
         * are untouched; the abandoned round is replayed, not skipped.
         * The park-duration gate is also what stops a lone survivor from
         * re-rolling a bad board at will by hopping their own connection.
         */
        const parkedMs = room.pausedAt !== undefined
          ? Date.now() - room.pausedAt
          : 0;
        if (!room.roundVirgin && parkedMs >= RoomManager.RESEED_MIN_PARK_MS) {
        ...
        // Resume: the lone returner takes the board back. They cannot be a
        // bystander in an empty room, whatever a previous mid-round rejoin
        // marked them.
  ...
  sweep(nowMs) {
    ...
    for (const [code, room] of this.rooms.entries()) {
      ...
      // Un-wedge stalled shop intermissions.
      if (room.phase === 'shopping' && hasConnected &&
          room.shoppingStartedAt !== undefined &&
          nowMs - room.shoppingStartedAt > RoomManager.SHOP_TIMEOUT_MS) {
        const connected = Array.from(room.players.values()).filter(p => p.connected);
        room.readyForNextRound = room.readyForNextRound || new Set();
        connected.forEach(p => room.readyForNextRound.add(p.slot));
        const advanced = this.maybeBeginNextRound(room);
        if (advanced) {
          replies.push(...(advanced.replies || []));
          broadcasts.push(...(advanced.broadcasts || []));
        }
      ...
      // as a rejoin would; if everyone connected is a SPECTATOR (the
      // actor-of-record dropped and never came back), re-seed the round —
      // spectators-of-a-dead-world would otherwise wait here forever.
      if (room.phase === 'paused' && hasConnected &&
        ...
        if (eligible) {
          ...
          const reseeded = this.reseedCurrentRound(room);
          replies.push(...(reseeded.replies || []));
          broadcasts.push(...(reseeded.broadcasts || []));
        }
      }

      // A never-started lobby does not outlive MAX_CONNECTED_LOBBY_MS even
      // with its creator seated: held-open sockets were a free permanent
      // squat on the global room budget.
      const connectedLobbyExpired = room.phase === 'lobby' &&
        room.players.size < 2 &&
        nowMs - room.createdAt > RoomManager.MAX_CONNECTED_LOBBY_MS;

      // Universal rule: A room with any connected player is never swept —
      // except the expired single-seat lobby above.
      if (hasConnected && !connectedLobbyExpired) {
        continue;
      }

      if (connectedLobbyExpired) {
        this.rooms.delete(room.code);
        for (const p of room.players.values()) {
          if (p.connectionId) this.unbindConnection(p.connectionId, room);
          if (p.connectionId) {
            replies.push({
              to: p.connectionId,
              msg: {
...
// Aggregate room ceiling — see createRoom. High enough that squatting it
// requires sustained sockets across many real addresses (the per-IP
// connection cap and creation budget price each one), low enough to bound
// memory; the connected-lobby TTL below makes any squat temporary.
RoomManager.MAX_TOTAL_ROOMS = 500;
// A never-started lobby is reaped past this age even with its creator still
// connected: an honest host does not sit alone in a lobby for an hour, but
// a squatter's held socket used to pin a room forever.
RoomManager.MAX_CONNECTED_LOBBY_MS = 3600000; // 1 hour
// A dirty (already-fought) round is only re-seeded for a rejoiner once the
// room has been fully parked this long — quick blips resume instead, and a
// lone survivor cannot re-roll a bad board by hopping their own connection.
RoomManager.RESEED_MIN_PARK_MS = 30000;
...
// A player must finish their shopping inside this window.
RoomManager.SHOP_TIMEOUT_MS = 180000; // 3 minutes
