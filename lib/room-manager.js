...
const { ERRORS, sanitiseInventory } = require('./protocol');
const {
...
class RoomManager {
  ...
  start(connectionId, config) {
    ...
    // ROUND_START restates it. 'constant' keeps the minted value for the
    // whole match; the default redraws per round in startNextRound.
  ...
  shopDone(connectionId, payload) {
    ...
    if (payload && typeof payload === 'object' &&
      ...
      callerPlayer.cash = Math.floor(payload.cash);
    }
